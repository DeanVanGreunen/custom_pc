...
pub struct Flags {
    ...
    pub carry: bool,
    /// Set when the result has bit 31 set (negative in two's complement).
    pub negative: bool,
...
impl Flags {
    ...
    /// Update Z and N from a 32-bit result.
    ...
    pub fn set_add(&mut self, a: u32, b: u32, cin: u64) {
        let full = (a as u64) + (b as u64) + cin;
        let result = full as u32;
        self.zero     = result == 0;
        self.negative = result & 0x8000_0000 != 0;
        self.carry    = full > 0xFFFF_FFFF;
        // Overflow: same-sign inputs produced opposite-sign output.
        let sa = (a & 0x8000_0000) != 0;
        let sb = (b & 0x8000_0000) != 0;
        let sr = (result & 0x8000_0000) != 0;
        self.overflow = (sa == sb) && (sa != sr);
    }

    /// Update Z, N, C, and V after a subtraction: `a - b`.
    pub fn set_sub(&mut self, a: u32, b: u32) {
        let full = (a as u64).wrapping_add((!b as u64).wrapping_add(1));
        let result = full as u32;
        self.zero     = result == 0;
        self.negative = result & 0x8000_0000 != 0;
