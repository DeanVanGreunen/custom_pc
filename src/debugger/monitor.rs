...
impl Monitor {
        ...
        loop {
            ...
            match cmd {
                ...
                "step" | "s" => {
                    ...
                    for _ in 0..n {
                        ...
                        if self.breakpoints.is_set(machine.cpu.pc) {
                            println!("  breakpoint at 0x{:04X}", machine.cpu.pc);
                            break;
                        }
                    }
                ...
                "dis"  => {
                    ...
                    let addr = parse_u16(addr_s).unwrap_or(machine.cpu.pc);
                    let n    = rest.trim().parse::<usize>().unwrap_or(8);
                    for (a, bytes, text) in disassemble(&machine.mem, addr, n) {
                        let hex: String = bytes.iter().map(|b| format!("{b:02X} ")).collect();
                        println!("  {:04X}  {:12}{}", a, hex, text);
                    }
                }
                "mem"  => {
                    let (addr_s, rest) = arg.split_once(' ').unwrap_or((arg, ""));
                    let addr = parse_u16(addr_s).unwrap_or(0);
                    let n    = rest.trim().parse::<usize>().unwrap_or(64);
                    let slice: Vec<u8> = (0..n as u16)
                        .map(|i| machine.mem.read_byte(addr.wrapping_add(i)))
                        .collect();
                    print!("{}", hex_dump(&slice, addr));
                }
                "bp"   => if let Some(a) = parse_u16(arg) {
                    self.breakpoints.add(a);
                    println!("  breakpoint added at 0x{a:04X}");
                }
                "bpd"  => if let Some(a) = parse_u16(arg) {
    ...
    fn print_regs(&self, machine: &Machine) {
        ...
        for row in 0..4 {
            for col in 0..4 {
                let i = row * 4 + col;
                print!("  r{i:<2} = 0x{:04X}", cpu.regs.get(i as u8));
            }
            println!();
        }
        let f = &cpu.flags;
        println!("  PC=0x{:04X}  Z={} C={} N={} V={}  cycles={}",
